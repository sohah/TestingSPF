...
public class SymbolicSequenceListener extends PropertyListenerAdapter implements PublisherExtension {
    ...
    public SymbolicSequenceListener(Config conf, JPF jpf) {
        ...
        assert (conf.getString("benchmark") != null) : "must define a benchmark. Failing";
        benchmark = conf.getString("benchmark");

        assert (conf.getString("testCaseFor") != null) : "must define a method for testCaseFor. Failing";
        testCaseForMethod = conf.getString("testCaseFor");

        try {
            fw = new FileWriter(("../testCase/testCases_" + benchmark + ".txt"), false);
            bw = new BufferedWriter(fw);
            out = new PrintWriter(bw);
            out.close();
        } catch (IOException e) {
            e.printStackTrace();
            assert false : "problem finding the testcase file to output to. Failing";
        }
